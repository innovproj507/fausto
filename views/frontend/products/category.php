...
<div class="bg-secondary py-10">
    <div class="container mx-auto px-4">
        <h1 class="font-display text-4xl font-bold text-white uppercase tracking-wide"><?= sanitize($category['name']) ?></h1>
        <?php if (!empty($category['description'])): ?>
            <p class="text-gray-300 mt-2"><?= sanitize($category['description']) ?></p>
        <?php endif; ?>
...
<div class="container mx-auto px-4 py-8">
    ...
    <?php if (isset($pagination) && $pagination['last_page'] > 1): ?>
        <?php
            ...
            $pageUrl = function (int $i) use ($category) {
                return url('/category/' . $category['slug'] . '?' . http_build_query(['page' => $i]));
            };
